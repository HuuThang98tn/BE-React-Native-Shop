...
class TypeProductController {
  ...
  mEditProduct(req, res, next) {
    ...
    TypeProduct.findById(req.params.id, (err, tb_typeproducts) => {
      if (err) {
        res.status(400).json({
          message: "Đã có lỗi xảy ra",
          err,
        ...
        const img = tb_typeproducts.imageTypeProduct;
        if (req.file) {
          // kiểm tra xem trường này nhập vào có phải là 1 file hay không ?
          ...
          mFormData.imageTypeProduct = tb_typeproducts.imageTypeProduct;
        }
  ...
  mDestroyProduct(req, res, next) {
    TypeProduct.findById(req.params.id, (err, tb_typeproducts) => {
      if (err) {
        ...
        // console.log("tb_typeproducts", tb_typeproducts);

        TypeProduct.deleteOne(tb_typeproducts, (err) => {
          // console.log(tb_typeproducts);
          if (err) {
  ...
  mListProduct(req, res, next) {
    ...
    TypeProduct.find({}, function (err, tb_typeproducts) {
      for (let i = 0; i < tb_typeproducts.length; i++) {
        tb_typeproducts[i].imageTypeProduct =
          process.env.BASE_URL + `${tb_typeproducts[i].imageTypeProduct}`;
      }
      if (!err) {
        res.json(tb_typeproducts);
      } else {
